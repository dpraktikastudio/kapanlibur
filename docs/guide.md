...
- Show **perencana cuti** (leave-window ranking + carousel) with **penawaran** beside it on desktop, **today’s holiday status** in a full-width row below, a **navigable “next/previous” holiday** line in the **sticky promo bar**, **one month at a time** of upcoming rows, and a **full-year calendar**.
...
- **Top split** (`#home-top-split`): **`aria-busy`** until holidays load. **Row 1 — CSS grid** `lg:grid-cols-3`: **left** `lg:col-span-2` = **`#cuti-optimizer-section`**; **right** `lg:col-span-1` = `<aside aria-label="Penawaran">` + **`#home-aside-promo`** (`lg:sticky lg:top-40`). **Row 2 — full width:** `<section aria-label="Status hari ini">` + **`#home-today-card-shell`** + **`#hero-content.hero-today-card`**. Single-column flow: **cuti → promo → status**.
...
- **Cards:** month badge, tanggal cuti / rentang / span, **Rincian hari** (`scheduleLines`), **Ide aktivitas**, optional **Cek Promo Tiket & Hotel** (`promoHref` on the matching **`ACTIVITY_BY_SPAN`** bucket; empty = omit), **Bagikan** (plain-text blurb + Unicode-bold span total in share line when applicable).
- **Results reveal:** On successful run, **`#cuti-optimizer-results`** gets a one-shot **`cuti-optimizer-results-reveal`** animation (`cutiResultsReveal` keyframes); not re-fired on carousel **reorder** only. Respects **`prefers-reduced-motion`**.
